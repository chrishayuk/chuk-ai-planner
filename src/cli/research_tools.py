...
async def register_enhanced_tools(processor, tools_dict, search_tool, visit_url_tool):
        ...
        for idx, result in enumerate(results.get("results", [])):
            ...
            if url and not url.startswith("data:"):  # Skip data URLs
                extracted_urls.append(
                    {"index": idx + 1, "title": result.get("title", ""), "url": url}
                )
    ...
    async def enhanced_visit_url_wrapper(args):
        ...
        if (
            url.startswith("URL_FROM_STEP_")
            or not url
            or (not url.startswith("http") and not url.startswith("https"))
        ):
        ...
        try:
            ...
            if any(
                domain in url.lower()
                for domain in ["linkedin.com", "twitter.com", "facebook.com"]
            ):
                ...
                if username and len(username) > 2:
                    ...
                    search_results = await enhanced_search_wrapper(
        ...
        if step_num in extracted_urls and extracted_urls[step_num]:
            ...
            if result_idx < len(extracted_urls[step_num]):
                ...
                if real_url:
                    ...
                    print(
                        f"⚠️ No URL found in result {result_idx + 1} from step {step_num}"
                    )
            else:
                print(
                    f"⚠️ Result index {result_idx + 1} out of range for step {step_num}"
                )
        else:
            ...
            if all_urls:
                ...
                print(
                    f"🔄 No URLs from step {step_num}, using URL from other step: {real_url}"
                )
                return real_url
            else:
