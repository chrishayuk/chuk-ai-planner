...
def validate_step(step: Dict[str, Any]) -> Tuple[bool, str]:
    ...
    if miss:
        return False, f"{tool}: missing {miss}"
    if extra:
        return False, f"{tool}: unknown {extra}"
    if bad:
        return False, f"{tool}: invalid {bad}"
    return True, ""
...
async def create_plan_for_entities(
    agent: PlanAgent, topic: str, entities: List[str]
) -> Optional[Dict]:
...
async def create_follow_up_plan(
    agent: PlanAgent, goal: str, search_results: List[Dict]
) -> Optional[Dict]:
    ...
    return {"title": f"Explore Key Sources for {goal}", "steps": url_steps}


async def extract_new_directions(
    agent: PlanAgent, goal: str, all_results: List[Dict]
) -> List[str]:
...
async def execute_plan(
    plan_json: Dict, session_id: str, tracker: ResearchTracker
) -> List[Dict]:
...
async def create_comprehensive_summary(results: list[dict[str, Any]], goal: str) -> str:
    ...
    for result in results:
        if "results" in result:  # Search result
            ...
            website_content += (
                f"CONTENT: {result.get('first_200_chars', 'No content')}\n\n"
            )
...
async def run(user_prompt: str) -> None:
    ...
    website_plan = await create_follow_up_plan(
        agent, user_prompt, tracker.search_results
    )
    ...
    new_directions = await extract_new_directions(
        agent, user_prompt, tracker.all_results
    )
    ...
    if new_directions:
        ...
        if deep_research_plan and deep_research_plan.get("steps"):
            ...
            if follow_up_plan and follow_up_plan.get("steps"):
                ...
                follow_up_results = await execute_plan(
                    follow_up_plan, session.id, tracker
                )
