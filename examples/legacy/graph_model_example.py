...
from chuk_ai_planner.graph import ParentChildEdge, NextEdge, PlanEdge, StepEdge

# Create a graph representing a conversation with planning and tool execution

# 1. Create a session
session = SessionNode(data={"title": "Weather Research Session", "user_id": "user123"})
...
session_to_msg = ParentChildEdge(src=session.id, dst=user_msg.id)
...
plan_to_session = ParentChildEdge(src=session.id, dst=assistant_plan.id)
...
step3 = PlanStep(data={"description": "Summarize findings", "index": 3})
...
task_run1 = TaskRun(
    data={"executor": "weather_service", "cost": 0.005, "latency_ms": 250}
)

task_run2 = TaskRun(
    data={"executor": "weather_service", "cost": 0.008, "latency_ms": 320}
)
...
def print_graph_structure():
    ...
    message_edges = [
        ...
        for e in edges
        if e.src == session.id
        and (e.dst == user_msg.id or e.dst == assistant_msg.id or e.dst == final_msg.id)
    ]
        ...
        if message:
            if message.kind.value == "user_message":
                ...
                tool_edges = [
                    e
                    for e in edges
                    if e.src == message.id and e.kind.value == "parent_child"
                ]
                for j, tool_edge in enumerate(tool_edges):
                    ...
                    if tool and tool.kind.value == "tool_call":
                        ...
                        print(
                            f"        └── {tool.data.get('name', 'unnamed')}({tool.data.get('args', {})})"
                        )
    ...
    step_edges = [
        e
        for e in edges
        if e.src == assistant_plan.id and e.kind.value == "parent_child"
    ]
    ...
    for i, edge in enumerate(step_edges):
        ...
        if step and step.kind.value == "plan_step":
            print(
                f"{prefix} Step {step.data.get('index')}: {step.data.get('description')}"
            )

            # Find executions linked to this step
            exec_edges = [
                e for e in edges if e.src == step.id and e.kind.value == "plan_link"
            ]
...
if __name__ == "__main__":
    ...
    try:
        ...
        for node in nodes:
            G.add_node(
                node.id, kind=node.kind.value, label=f"{node.kind.value}:{node.id[:6]}"
            )
        ...
        nx.draw(
            G, pos, with_labels=False, node_color=colors, node_size=500, arrows=True
        )
