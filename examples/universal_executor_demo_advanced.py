    ...
    if source == "weather":
        return {
            "data": [
                ...
                },
                {
                    ...
                    "conditions": random.choice(["Cloudy", "Rainy", "Foggy"]),
                },
                {
                    "city": "Tokyo",
        ...
        return {
            "data": [
                {
                    "symbol": "AAPL",
                    "price": round(random.uniform(170, 190), 2),
                    "change": round(random.uniform(-5, 5), 2),
                },
                {
                    "symbol": "MSFT",
                    "price": round(random.uniform(340, 360), 2),
...
def create_main_plan(subplan_ids: Dict[str, str]) -> UniversalPlan:
    ...
    combine_step_id = plan.add_function_step(
        ...
        depends_on=[
            subplan_step_ids["weather"],
            subplan_step_ids["stocks"],
            subplan_step_ids["news"],
        ],
    )
...
async def subplan_execution_tool(
    args: Dict[str, Any], registry: SimplePlanRegistry, executor: UniversalExecutor
) -> Dict[str, Any]:
    ...
    if not result["success"]:
        raise RuntimeError(
            f"Subplan execution failed: {result.get('error', 'Unknown error')}"
        )

    print(
        f"🔄 Subplan {plan_id} completed. Variables: {list(result['variables'].keys())}"
    )
...
async def main():
    ...
    steps = [
        node
        for node in executor.graph_store.nodes.values()
    ...
    if (
        "final_report" in results["variables"]
        and isinstance(results["variables"]["final_report"], dict)
        and "title" in results["variables"]["final_report"]
    ):
        results_file = "multi_source_analysis_results.json"
        with open(results_file, "w") as f:
            json.dump(
                make_json_serializable(results["variables"]["final_report"]),
                f,
                indent=2,
            )
