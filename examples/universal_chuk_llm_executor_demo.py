#!/usr/bin/env python
# examples/universal_chuk_llm_executor_demo.py
"""
Universal chuk_llm-to-Execution Demo
===================================
...
to executed plan with results, now using chuk_llm instead of OpenAI directly.
"""
...
async def call_llm_live(task: str, provider: str = "openai") -> Dict[str, Any]:
...
def convert_to_universal_plan(llm_json: Dict[str, Any]) -> UniversalPlan:
    ...
    plan = UniversalPlan(
        ...
        description="Generated from chuk_llm input",
        tags=["llm-generated", "chuk_llm"]
    )
    
    # Add metadata about the source
    plan.add_metadata("source", "chuk_llm")
...
# -------------------------------------------------------------------- main flow
async def main() -> None:
    print("🚀 Universal chuk_llm-to-Execution Demo\n" + "=" * 45)
    
    # Allow the user to customize the task via command line
    parser = argparse.ArgumentParser(description="chuk_llm to Universal Plan Demo")
    ...
    if args.live:
        try:
            llm_json = await call_llm_live(task, args.provider)
        except Exception as e:
    ...
    try:
        ...
        output_file = "chuk_llm_plan_results.json"
