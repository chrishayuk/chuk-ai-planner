...
from chuk_ai_planner.core.graph.types import NodeType, EdgeType, StepStatus
...
async def main():
    ...
    await graph.add_edge(ParentChildEdge(src=plan.id, dst=step3.id))
    print("✅ Linked steps to plan")

    # 4. Create Step Edges (Dependencies)
    # Step 2 depends on Step 1
    await graph.add_edge(StepEdge(src=step1.id, dst=step2.id))
    # Step 3 depends on Step 2
    await graph.add_edge(StepEdge(src=step2.id, dst=step3.id))
    ...
    # Get all plan nodes
    plans = await graph.get_nodes_by_kind(NodeType.PLAN)
    print(f"\n📊 Found {len(plans)} plan(s)")

    # Get all step nodes
    steps = await graph.get_nodes_by_kind(NodeType.PLAN_STEP)
    print(f"📊 Found {len(steps)} step(s)")

    # Get children of plan (using ParentChild edges)
    plan_children = await graph.get_edges(src=plan.id, kind=EdgeType.PARENT_CHILD)
    print(f"📊 Plan has {len(plan_children)} child steps")

    # Get step dependencies (StepEdge)
    step2_deps = await graph.get_edges(dst=step2.id, kind=EdgeType.STEP_ORDER)
    ...
    print("\n🔍 Step 1 Details:")
    print(f"   Description: {step1.description}")  # Not step1.description!
    ...
    print("✅ Direct field access - plan.title, not plan.title")
