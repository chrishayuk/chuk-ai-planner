...
class ToolProcessorWrapper:
    ...
    def __init__(self, registry):
        ...
        self.strategy = InProcessStrategy(
            registry, default_timeout=10.0, max_concurrency=4
        )
    ...
    async def execute_tool_call(
        ...
        call = ToolCall(tool=tool_name, namespace="demo", arguments=args)
...
async def execute_plan(g, plan, tool_wrapper):
    ...
    async def guarded_execute_tool(
        ...
        async with _sema:
            return await tool_wrapper.execute_tool_call(
                tool_call, parent_event_id, assistant_node_id
            )
...
async def demo_with_strategy_comparison(g, plan):
    ...
    inprocess_wrapper.strategy = InProcessStrategy(
        registry, default_timeout=10.0, max_concurrency=4
    )
    inprocess_wrapper.executor = ToolExecutor(
        registry=registry, strategy=inprocess_wrapper.strategy
    )
    ...
    try:
        ...
        subprocess_wrapper.strategy = SubprocessStrategy(
            registry, max_workers=4, default_timeout=10.0
        )
        subprocess_wrapper.executor = ToolExecutor(
            registry=registry, strategy=subprocess_wrapper.strategy
        )
