...
def create_weather_report(analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    report = {
        ...
        "recommendations": [
            ...
            if len(analysis.get("condition_distribution", {})) > 2
            else "Weather is fairly consistent",
            "Bring rain gear"
            if "Rainy" in analysis.get("condition_distribution", {})
            else "No need for rain gear",
            "Dress for warm weather"
            if analysis.get("average_temperature", 0) > 75
            else "Pack some warmer clothes",
        ],
    }
...
def format_visualization_data(
    weather_data: Dict[str, Any], analysis: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    for condition, count in analysis.get("condition_distribution", {}).items():
        conditions.append({"condition": condition, "count": count})
...
async def run_weather_analysis_pipeline(locations: List[str]) -> Dict[str, Any]:
    ...
    report_task = asyncio.create_task(
        asyncio.to_thread(create_weather_report, analysis)
    )
    viz_task = asyncio.create_task(
        asyncio.to_thread(format_visualization_data, weather_data, analysis)
    )
