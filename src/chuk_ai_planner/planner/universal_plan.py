...
class UniversalPlan(ChukPlan):
        ...
        if result_variable:
            ...
            self._graph.add_edge(
                CustomEdge(
                    ...
                    metadata={"variable": result_variable},
                )
            )
    ...
    def add_plan_step(
        ...
        tool_call = ToolCall(
            name="subplan", args={"plan_id": plan_id, "args": args or {}}
        )
        self._graph.add_node(tool_call)
    ...
    def add_function_step(
        ...
        tool_call = ToolCall(
            name="function", args={"function": function, "args": args or {}}
        )
        self._graph.add_node(tool_call)
        ...
        if self._indexed:  # If the plan has been indexed
            ...
            for node in self._graph.nodes.values():
                if (
                    ...
                    and node.description == title  # Use description instead of title
                    and node.index.startswith(str(len(current.children)))
                ):
                    step_id = node.id
                    break
        ...
        if "steps" in data:
            for step_data in data["steps"]:
                ...
                for node in plan._graph.nodes.values():
                    if node.__class__.__name__ == "PlanStep" and node.index == index:
                        step_node = node
                        break
