...
from chuk_ai_planner.graph import (
    EdgeType, ParentChildEdge, NextEdge, PlanEdge, StepEdge, GraphEdge
)
...
def create_testing_session():
    ...
    edges = [
        ...
        *[ParentChildEdge(src=task_runs[i].id, dst=test_summaries[i].id) 
        ...
        GraphEdge(kind=EdgeType.CUSTOM, src=test_steps[1].id, dst=test_steps[2].id,
                 data={"execution_type": "parallel", "group": "test_execution"}),
        GraphEdge(kind=EdgeType.CUSTOM, src=test_steps[1].id, dst=test_steps[3].id,
                 data={"execution_type": "parallel", "group": "test_execution"}),
        GraphEdge(kind=EdgeType.CUSTOM, src=test_steps[1].id, dst=test_steps[4].id,
                 data={"execution_type": "parallel", "group": "test_execution"}),
    ]
