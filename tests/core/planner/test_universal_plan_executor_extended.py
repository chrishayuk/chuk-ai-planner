...
from chuk_ai_planner.core.graph import ToolCall, PlanStep, PlanLinkEdge, CustomEdge
...
class TestEnhancedVariableResolution:
    ...
    def test_resolve_nested_variables_simple(self, executor):
        ...
        variables = {"user": {"name": "Alice", "age": 30}, "config": {"timeout": 30}}
    ...
    def test_resolve_nested_variables_deep(self, executor):
        ...
        variables = {"data": {"user": {"profile": {"settings": {"theme": "dark"}}}}}

        result = executor._resolve_vars(
            "${data.user.profile.settings.theme}", variables
        )
        assert result == "dark"
    ...
    def test_resolve_nested_variables_in_list(self, executor):
        ...
        input_list = ["${user.skills}", "${config.features}", "static_item"]
...
class TestDuplicateExecutionPrevention:
    ...
    async def test_tool_call_deduplication(self, executor, graph_store):
        ...
        await graph_store.add_edge(PlanLinkEdge(src=step1.id, dst=tool_call.id))
        await graph_store.add_edge(PlanLinkEdge(src=step2.id, dst=tool_call.id))
...
class TestResultVariableManagement:
    ...
    @pytest.mark.asyncio
    async def test_find_result_variable(self, executor, graph_store):
        ...
        result_var = await executor._find_result_variable(step.id, tool_call.id)
        assert result_var == "my_result"

        # Test with non-existent variable
        result_var = await executor._find_result_variable("nonexistent", tool_call.id)
...
class TestComplexPlanExecution:
    ...
    async def test_complex_variable_flow(self, executor, complex_plan):
        ...
        step1 = await complex_plan.add_tool_step(
            ...
            tool="user_processor",
            args={"name": "${user_data.name}", "skills": "${user_data.profile.skills}"},
            result_variable="user_info",
        )

        step2 = await complex_plan.add_tool_step(
        ...
        await complex_plan.add_function_step(
            title="Generate report",
            function="report_generator",
            args={"user_info": "${user_info}", "config_info": "${config_info}"},
    ...
    async def test_dependency_execution_order(self, executor, graph_store):
        ...
        step_a = await plan.add_tool_step(
            "Step A", "tracking_tool", {"step_name": "A"}, result_variable="result_a"
        )

        # Step B (no dependencies, parallel with A)
        step_b = await plan.add_tool_step(
            "Step B", "tracking_tool", {"step_name": "B"}, result_variable="result_b"
        )

        # Step C (depends on A and B)
        step_c = await plan.add_tool_step(
            "Step C",
            "tracking_tool",
            {"step_name": "C"},
...
class TestErrorHandlingAndRecovery:
    ...
    async def test_tool_error_propagation(self, executor, graph_store):
        ...
        await plan.add_tool_step(
            "Failing step", "failing_tool", {"error_type": "value"}
        )
    ...
    async def test_unknown_tool_error(self, executor, graph_store):
        ...
        await plan.add_tool_step("Unknown tool", "nonexistent_tool", {})
        ...
        with patch.dict("sys.modules", {"chuk_ai_planner.models.base": MagicMock()}):
            ...
            with patch.object(executor, "_get_json_serializable_data") as mock_method:
                ...
                mock_method.side_effect = (
                    lambda x: list(x) if isinstance(x, MockReadOnlyList) else x
                )
    ...
    def test_json_serializable_nested_frozen(self, executor):
        ...
        data = {
            ...
            "frozen": MappingProxyType(
                {"inner": "data", "nested_frozen": MappingProxyType({"deep": "value"})}
            ),
            "list_with_frozen": ["normal_item", MappingProxyType({"item": "frozen"})],
        }
...
class TestConcurrency:
    ...
    async def test_shared_executor_state(self, graph_store):
        ...
        await plan1.add_tool_step(
            "Step 1", "stateful_tool", {}, result_variable="result1"
        )

        plan2 = UniversalPlan("Plan 2")
        await plan2.add_tool_step(
            "Step 2", "stateful_tool", {}, result_variable="result2"
        )
...
class TestJSONSerializationHelpers:
    """Test JSON serialization helper methods"""

    @pytest.mark.asyncio
    async def test_json_serializable_mapping_proxy(self):
        """Test MappingProxyType conversion"""
        from types import MappingProxyType

        executor = UniversalExecutor()
        data = MappingProxyType({"key": "value", "nested": {"a": 1}})
        result = executor._get_json_serializable_data(data)
    ...
    @pytest.mark.asyncio
    async def test_json_serializable_regular_dict(self):
        """Test regular dict preservation"""
        executor = UniversalExecutor()
    ...
    @pytest.mark.asyncio
    async def test_json_serializable_nested_frozen(self):
        """Test nested frozen structures"""
        from types import MappingProxyType

        executor = UniversalExecutor()
        data = {"outer": MappingProxyType({"inner": "value"})}
        result = executor._get_json_serializable_data(data)
...
class TestErrorPropagation:
    ...
    @pytest.mark.asyncio
    async def test_execute_plan_with_missing_tool(self, graph_store):
        """Test execution with unregistered tool"""
        executor = UniversalExecutor(graph_store)

        plan = UniversalPlan("Error Plan", graph=graph_store)
        await plan.add_tool_step("Missing tool", "nonexistent_tool", {})
    ...
    @pytest.mark.asyncio
    async def test_execute_plan_with_missing_function(self, graph_store):
        """Test execution with unregistered function"""
        executor = UniversalExecutor(graph_store)

        plan = UniversalPlan("Error Plan", graph=graph_store)
        await plan.add_function_step("Missing function", "nonexistent_func", {})
...
class TestVariableResolutionEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_resolve_vars_with_none_value(self):
        """Test variable resolution with None values"""
        executor = UniversalExecutor()
        variables = {"null_var": None}

        result = executor._resolve_vars("${null_var}", variables)
        assert result is None

    @pytest.mark.asyncio
    async def test_resolve_vars_with_boolean(self):
        """Test variable resolution with boolean values"""
        executor = UniversalExecutor()
        variables = {"bool_var": True, "false_var": False}

        result1 = executor._resolve_vars("${bool_var}", variables)
        result2 = executor._resolve_vars("${false_var}", variables)

        assert result1 is True
        assert result2 is False

    @pytest.mark.asyncio
    async def test_resolve_vars_nested_dicts(self):
        """Test variable resolution with deeply nested dicts"""
        executor = UniversalExecutor()
        variables = {"level1": {"level2": {"level3": {"level4": "deep_value"}}}}

        result = executor._resolve_vars("${level1.level2.level3.level4}", variables)
        assert result == "deep_value"

    @pytest.mark.asyncio
    async def test_resolve_vars_mixed_content(self):
        """Test variable resolution with mixed content"""
        executor = UniversalExecutor()
        variables = {"name": "Alice", "age": 30}

        # Mixed list with variables and literals
...
class TestSessionManagement:
    ...
    async def test_ensure_session_idempotent(self):
        """Test that ensure_session can be called multiple times"""
        executor = UniversalExecutor()
        ...
        assert session1 is session2  # Should be same session

    @pytest.mark.asyncio
    async def test_processor_initialized_after_session(self):
        """Test that processor is initialized with session"""
        executor = UniversalExecutor()
