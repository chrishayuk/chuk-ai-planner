...
async def calculation_tool_call(
    tool_call_dict: Dict[str, Any], parent_event_id: str, assistant_node_id: str
) -> Dict[str, Any]:
...
async def create_demo_plan() -> tuple[Plan, str]:
    ...
    plan.step(
        "Calculate temperature difference", after=["1", "2"]
    )  # Depends on both weather steps
    plan.up()
...
async def main():
    ...
    for event in session_manager.events:
        ...
        message_summary = (
            str(event.message)[:100] + "..."
            if len(str(event.message)) > 100
