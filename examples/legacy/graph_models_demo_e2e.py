...
def demo_node_creation():
    ...
    user_msg = UserMessage(
        data={"content": "Hello, I need help with a data analysis task"}
    )
    ...
    step1 = PlanStep(data={"index": "1", "description": "Load and examine the dataset"})

    step2 = PlanStep(
        data={"index": "2", "description": "Clean and preprocess the data"}
    )
    ...
    task_run = TaskRun(
        data={"success": True, "execution_time": 2.3, "memory_used": "45MB"}
    )

    summary = Summary(
        data={"content": "Successfully loaded dataset with 1000 rows and 15 columns"}
    )
...
def demo_edge_creation():
    ...
    edges = [
        ParentChildEdge(
            src=session.id, dst=user_msg.id, data={"relationship": "contains"}
        ),
        ...
        StepEdge(
            src=step1.id, dst=step2.id, data={"dependency": "step1_must_complete_first"}
        ),
        ...
        GraphEdge(
            ...
            src=step1.id,
            dst=step2.id,
            data={"custom_type": "data_flow", "variables": ["dataset", "cleaned_data"]},
        ),
    ]
...
def demo_graph_store_operations():
    ...
    assistant_msg = AssistantMessage(
        data={"content": "I'll analyze the Q3 sales data for you."}
    )
    ...
    tools = [
        ...
        ToolCall(
            data={
                ...
                "args": {"columns": ["revenue", "units"]},
            }
        ),
        ToolCall(
            data={
                "name": "create_chart",
                "args": {"type": "line", "x": "month", "y": "revenue"},
            }
        ),
        ToolCall(
            data={"name": "generate_report", "args": {"template": "executive_summary"}}
        ),
    ]
    ...
    session_children = graph.get_edges(src=session.id, kind=EdgeType.PARENT_CHILD)
    ...
    # Find plan steps
    plan_steps = graph.get_edges(src=plan.id, kind=EdgeType.PARENT_CHILD)
    ...
    print("\n⚡ Step dependencies:")
    for step in steps:
        deps = graph.get_edges(dst=step.id, kind=EdgeType.STEP_ORDER)
        if deps:
            for dep_edge in deps:
                dep_step = graph.get_node(dep_edge.src)
                print(
                    f"   Step {dict(step.data).get('index')} depends on Step {dict(dep_step.data).get('index')}"
                )
...
def demo_node_immutability():
    ...
    try:
        tool_call.kind = NodeType.SUMMARY
...
def demo_complex_graph_scenario():
    ...
    research_tools = [
        ...
        ToolCall(
            data={
                ...
                "args": {"input_papers": 40, "synthesis_method": "thematic_analysis"},
                "result": {
                    "key_findings": [
    ...
    all_nodes = (
        [session, user_msg, assistant_msg, research_plan]
        + research_steps
        + research_tools
        + task_runs
        + summaries
    ...
    edges = [
        ...
        # Plan structure
        *[
            ParentChildEdge(src=research_plan.id, dst=step.id)
    ...
    print("📊 Complex Research Graph Created:")
    print(f"   Total nodes: {len(graph.nodes)}")
    print(f"   Total edges: {len(graph.edges)}")
    print(f"   Research steps: {len(research_steps)}")
    print(f"   Tools executed: {len(research_tools)}")
    print(
        f"   Successful task runs: {sum(1 for tr in task_runs if tr.data.get('success'))}"
    )

    # Analyze the research workflow
    print("\n🔬 Research Workflow Analysis:")
    total_papers = sum(
        tool.data.get("result", {}).get("papers_found", 0)
        for tool in research_tools[:2]
    )
...
def demo_visualization():
    ...
    plan = PlanNode(
        data={"title": "Simple Demo Plan", "description": "A plan for demonstration"}
    )
