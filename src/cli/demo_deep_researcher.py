...
from cli.research_tools import (
    register_enhanced_tools,
    resolve_url_placeholders,
    format_tool_result,
)
from cli.research_summarizer import (
    ...
    for step in steps:
        ...
        try:
            ...
            if tool_name == "search" and "extracted_urls" in result:
                print(
                    f"📋 Storing {len(result['extracted_urls'])} URLs from step {step['index']}"
                )
    ...
    if not any(r.tool == "visit_url" for r in results):
        ...
        for step_idx, url_list in extracted_urls.items():
            for url_info in url_list:
                all_urls.append({"step": step_idx, "url_info": url_info})
            ...
            if url:
                ...
                try:
                    ...
                    result_obj = type(
                        "ToolResult",
                        (),
                        {"tool": "visit_url", "args": {"url": url}, "result": result},
                    )
...
def ensure_url_visits_in_plan(plan_json):
    ...
    search_steps = [
        i + 1 for i, step in enumerate(steps) if step.get("tool") == "search"
    ]
        ...
        for step in steps:
            ...
            if step.get("tool") == "search":
                ...
                visit_step = {
                    ...
                    "tool": "visit_url",
                    "args": {"url": f"URL_FROM_STEP_{search_idx}_RESULT_1"},
...
def cli():
    ...
    parser.add_argument(
        "--steps",
        type=int,
        default=DEFAULT_CONFIG["max_searches_per_round"],
