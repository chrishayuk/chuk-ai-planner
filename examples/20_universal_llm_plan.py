...
def create_tool_implementations():
    ...
    async def brew_coffee_tool(**kwargs):
        ...
        await asyncio.sleep(0.1)
        return {
            "status": "Coffee brewed perfectly",
            "strength": "medium",
            "aroma": "excellent",
        }
        ...
        if result["success"]:
            ...
            for var_name, var_value in result["variables"].items():
                if var_name == "result_4":  # Weather result
                    weather = var_value
                    print(
                        f"🌤️ Weather in {weather.get('location')}: {weather.get('temperature')}°F, {weather.get('conditions')}"
                    )
                    ...
                    search = var_value
                    print(
                        f"🔍 Found {search.get('result_count')} search results for '{search.get('query')}'"
                    )
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--live", action="store_true", help="Use real OpenAI rather than the simulator"
    )
