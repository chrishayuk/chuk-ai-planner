...
def create_testing_session():
    ...
    test_response = AssistantMessage(
        data={
    ...
    all_nodes = (
        [session, test_request, test_response, test_plan]
        + test_steps
        + test_tools
        + task_runs
        + test_summaries
...
def analyze_test_execution(
    graph, session, test_plan, steps, tools, task_runs, summaries
):
    ...
    total_execution_time = sum(
        float(tr.data.get("execution_time", "0")) for tr in task_runs
    )
    total_memory_used = sum(
        float(tr.data.get("memory_used", "0MB").replace("MB", "")) for tr in task_runs
    )
    successful_steps = sum(1 for tr in task_runs if tr.data.get("success") == "true")

    print("🎯 Execution Summary:")
    print(
        f"   Total execution time: {total_execution_time:.1f} seconds ({total_execution_time / 60:.1f} minutes)"
    )
    ...
    for result in test_results:
        ...
        print(
            f"   {name:12} {result['passed']:2}/{result['total']:2} tests passed ({success_rate:5.1f}%) - {result['duration']:6.1f}s"
        )

    print(
        f"   {'TOTAL':12} {total_passed:2}/{total_tests:2} tests passed ({total_passed / total_tests * 100:5.1f}%)"
    )

    # Performance analysis
    print("\n⚡ Performance Analysis:")
    slowest_step = max(
        task_runs, key=lambda tr: float(tr.data.get("execution_time", "0"))
    )
    fastest_step = min(
        task_runs, key=lambda tr: float(tr.data.get("execution_time", "0"))
    )
    ...
    print(
        f"   Slowest step: {slowest_step.data.get('step_name')} ({slowest_step.data.get('execution_time')}s)"
    )
    print(
        f"   Fastest step: {fastest_step.data.get('step_name')} ({fastest_step.data.get('execution_time')}s)"
    )
    ...
    print(
        f"   Time saved by parallelization: {time_saved:.1f}s ({time_saved / sequential_time * 100:.1f}%)"
    )
    ...
    print(
        f"   Security vulnerabilities: {sec_result.get('vulnerabilities_found', 0)} (severity: {sec_result.get('severity', 'N/A')})"
    )
...
def demo_test_failure_analysis():
    ...
    print("\n📋 Recommended Actions:")
    print("   1. Fix payment validation regex in API layer")
    print("   2. Update UI element selectors for checkout button")
    print("   3. Investigate database performance under load")
    print("   4. Implement input sanitization for security issues")
    print("   5. Re-run affected test suites after fixes")
...
def demo_parallel_execution_visualization():
    ...
    for item in timeline:
        ...
        print(
            f"   {'':<20} {item['start']:>6.1f}s → {item['start'] + item['duration']:>6.1f}s ({item['duration']:>5.1f}s)"
        )
...
def main():
    ...
    try:
        ...
        graph, session, plan, steps, tools, task_runs, summaries = (
