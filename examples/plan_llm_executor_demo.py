#!/usr/bin/env python
# examples/plan_executor_llm_demo.py
"""
Plan Executor LLM-to-Execution Demo
===================================
...
async def weather_tool_call(
    tool_call_dict: Dict[str, Any], parent_event_id: str, assistant_node_id: str
) -> Dict[str, Any]:
    ...
    return {"id": tool_call_dict["id"], "result": result, "success": True}


async def calculator_tool_call(
    tool_call_dict: Dict[str, Any], parent_event_id: str, assistant_node_id: str
) -> Dict[str, Any]:
...
async def search_tool_call(
    ...
    results = [
        ...
        },
        {
            ...
            "url": f"https://research.com/{query.replace(' ', '_')}",
        },
        {
...
class ToolRegistry:
    ...
    async def process_tool_call(
        self, tool_call: Dict[str, Any], parent_event_id: str, assistant_node_id: str
    ) -> Any:
...
class SessionEventManager:
    ...
    def create_child_event(
        self, event_type: EventType, message: Dict[str, Any], parent_event_id: str
    ):
...
async def main(live: bool = False) -> None:
    print("🚀 Plan Executor LLM-to-Execution Demo\n" + "=" * 50)

    # Allow the user to customize the task via command line
    parser = argparse.ArgumentParser(description="LLM to Plan Executor Demo")
    parser.add_argument(
        "--task",
        type=str,
    ...
    try:
        output_file = "plan_executor_llm_results.json"
