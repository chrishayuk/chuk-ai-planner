...
def create_web_browsing_session():
            ...
            data={
                ...
                "result": {
                    ...
                    "top_matching_articles": [
                        ...
                        {
                            "title": "Google's Gemini Ultra breakthrough",
                            "relevance_score": 0.91,
                        },
                    ],
                    "filtered_keywords_found": [
                        "AI",
                        "GPT",
                        "Claude",
                        "Gemini",
    ...
    all_nodes = (
        [session, user_request, assistant_response, browsing_plan]
        + browsing_steps
        + browsing_tools
        + task_runs
        + step_summaries
    ...
    edges = [
        ...
        # Plan structure
        *[
            ParentChildEdge(src=browsing_plan.id, dst=step.id)
            for step in browsing_steps
        ],
        ...
        *[
        ...
        GraphEdge(
            ...
            src=browsing_tools[1].id,
            dst=browsing_tools[2].id,
            data={
                "data_type": "extracted_articles",
                "flow_direction": "output_to_input",
            },
        ),
        GraphEdge(
            kind=EdgeType.CUSTOM,
            src=browsing_tools[2].id,
            dst=browsing_tools[3].id,
            data={
                "data_type": "filtered_articles",
                "flow_direction": "output_to_input",
            },
        ),
        GraphEdge(
            kind=EdgeType.CUSTOM,
            src=browsing_tools[5].id,
            dst=browsing_tools[6].id,
...
def analyze_browsing_workflow(graph, session, browsing_plan, steps, tools, task_runs):
    ...
    print(
        f"   Slowest step: {slowest_step.data.get('step_name')} ({slowest_step.data.get('execution_time', 0):.1f}s)"
    )
    print(
        f"   Fastest step: {fastest_step.data.get('step_name')} ({fastest_step.data.get('execution_time', 0):.1f}s)"
    )
    ...
    print(
        f"   Categorization confidence: {tools[5].data['result']['top_ranked_articles'][0]['rank_score']:.1%}"
    )
...
def demo_error_handling_scenario():
    ...
    print("   Initial attempt: FAILED (timeout)")
    print("   Retry attempts: 3")
    print("   Recovery strategy: backup proxy")
    print("   Final result: SUCCESS")
    print(
        f"   Total time: {failed_task.data['execution_time'] + retry_task.data['execution_time']:.1f}s"
    )
